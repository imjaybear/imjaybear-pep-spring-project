...
public class SocialMediaController {
    ...
    public ResponseEntity<?> loginUserHandler(@RequestBody Account account) {
        ...
        if (existingAccount != null && existingAccount.getPassword().equals(account.getPassword())) {
            return 
              ResponseEntity.status(HttpStatus.OK).body(existingAccount);
        } else 
            return 
    ...
    public ResponseEntity<?> createMessageHandler(@RequestBody Message message) {
        ...
        if (createdMessage != null) {
            return 
              ResponseEntity.status(200).body(createdMessage);
        } else
            return 
    ...
    public ResponseEntity<?> retrieveMessageByIdHandler(@PathVariable("message_id") int messageId) {
        ...
        if (optionalMessage.isPresent()) {
            ...
            return 
              ResponseEntity.status(HttpStatus.OK).body(foundMessage);
        } else 
            return 
    ...
    public @ResponseBody ResponseEntity<Integer> deleteMessageByIdHandler(@PathVariable int message_id) {
        ...
        if (deletMessage != null) {
            ...
            return 
              ResponseEntity.status(200).body(lines.length);
        } else
            return 
              ResponseEntity.status(200).body(0);
    }
    ...
    public @ResponseBody ResponseEntity<List<Message>> getMessageByAccountId(@PathVariable int account_id) {
        ...
        return 
          ResponseEntity.status(200).body(messageList);
    }

    // UPDATE MESSAGE BY MESSAGE ID HANDLER
    @PatchMapping("/messages/{message_id}")
    public @ResponseBody ResponseEntity<Integer> updateMessageHandler(@PathVariable int message_id,
        ...
        if (message != null) {
            ...
            return 
              ResponseEntity.status(400).body(0);
    }
